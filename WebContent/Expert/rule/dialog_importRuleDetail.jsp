...
.fr_table_editRule tr
{
	line-height:25px;
}

.fr_table_editRule td
{
	padding-left:3px;
	padding-right:3px;
}
...
<div id="addRuleInfo_div" class="form">
	<form id="rule">
		<table id="add_rule_tb_dialog" class="fr_table" style="display:none">
			<tr>
				<td width='160'><span class="TextBold">遴选规则名称：</span></td>
				<td><input type="hidden" name="rule.ruleName" value="${rule.ruleName}"/></td>
				<td><input type="hidden" name="rule.id"/></td>
				<td><input type="hidden" id="batchName" disabled="disabled" size="30"/></td>
	...
	<form id="details">
		<table id="add_Rule_tb_dialog" class="fr_table_editRule"  style="float:left">
			<tr>
				<td width='165'>
			...
			<tr>
				<td>
					...
					不设置专家评价学科数
					<input type="hidden" name="detail4.operator" <c:if test="${detail4.conditionChecked}">checked</c:if>/>
		  ...
		  <table id="add_Rule_tb_dialog2" class="fr_table_editRule" style="float:right">	
			<tr>
...
$(document).ready(function() {
    ...
    $("#batchName").val(batchNameForImport);
    $("#batchId").val(batchIdForImport);
});
</script>