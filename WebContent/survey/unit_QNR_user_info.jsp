...
<script type="text/javascript">
      ...
      $(document).ready(function(){
    	 	...
    	 	jQuery("#user_list").jqGrid({
    		 	url:'${ContextPath}/InfoMaintain/unitQNRInfo_user_list',
           		datatype: 'json',
           		mtype:'GET',
    		...
    		$("#add_btn").click(function(){
    			 $.post("${ContextPath}/InfoMaintain/unitQNRInfo_surveyUserAdd", function(data){
   		   		  $('#dialog').empty();
   				  $('#dialog').append( data );
   			 	  $('#dialog').dialog({
   		   	  		    title:"新建调查对象",
   		   	  		    ...
   		   	  		    buttons:{
   		   	  		    	"确定":function(){
   		   	  		    		if($("#add_survey_user").valid()){
   			   	  	         		$.ajax({
   			    	  	      			type:'POST',
   			    	  	      			url:'${ContextPath}/InfoMaintain/unitQNRInfo_surveyUserSave',
    		...
    		$("#import_btn").click(function(){
   			 $.post("${ContextPath}/InfoMaintain/unitQNRInfo_importUser", function(data){
  		   		  $('#dialog').empty();
  				  $('#dialog').append( data );
  			 	  $('#dialog').dialog({
  		   	  		    title:"导入调查对象",
  		  ...
  		  $( "#dialog-confirm" ).dialog({
      	      ...
      	      buttons: {
      	        "确定": function() {
      	        	...
      	        	$.post('${ContextPath}/InfoMaintain/unitQNRInfo_deleteUser?userId=' +userId, function(data){
      	    			  if(data == true){
      	    				  alert_dialog("删除成功！");
      	    				  $("#user_list").setGridParam({url:'${ContextPath}/InfoMaintain/unitQNRInfo_user_list'}).trigger("reloadGrid");
      	    			  }
     ...
     //编辑用户
      function modifyUser(userId){
    	  ...
    	  $.post("${ContextPath}/InfoMaintain/unitQNRInfo_surveyUserEdit?userId= "+ userId, function(data){
		   		  $('#dialog').empty();
				  $('#dialog').append( data );
			 	  $('#dialog').dialog({
		   	  		    title:"编辑调查对象",
