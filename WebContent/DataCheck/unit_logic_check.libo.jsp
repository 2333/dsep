...
<div class="selectbar inner_table_holder">
	<div class="left">
		...
		</select><a id="logic_check_btn" class="button" href="#"><span
			class="icon icon-search"></span>开始检查</a><a id="logic_check_showResult"
			class="button" href="#"><span class="icon icon-search"></span>查看最新结果</a>
	</div>

	<div id="logic_check_rule_div" class="right">
		<a id="exportErrorAndWarnData" class="button" href="#"><span
			class="icon icon-export"></span>导出检查结果</a> <a id="logic_check_rule"
			class="button" href="#"><span class="icon icon-explanation"></span>逻辑检查规则</a>
	</div>
</div>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$("#showwarn").click(function() {
			...
			if (prevSelectedDiscId != currentSelectedDiscId) {
				$("#result_warn_tb").setGridParam({url : '${ContextPath}/check/unitLogicCheck_getCheckWarnList?discId='+ currentSelectedDiscId+ "&onlyGetWarn=yes", datatype:'json'}).trigger("reloadGrid");
		...
		$('#logic_config_tb').jqGrid(
		{
			...
			colModel : [ {name : 'ID',index : 'ID',width : 60,align : "center",hidden : true},
			             ...
			             {name : 'disciplineChsName',index : 'disciplineChsName',width : 100,align : "center"},
			             ...
			             {name : 'ENTITY_CHS_NAME',index : 'ENTITY_CHS_NAME',width : 200,align : "center",sorttype : "String",editable : true,hidden : true}, 
			             {name : 'USER_ID',index : 'USER_ID',width : 200,align : "center",sorttype : "String",editable : true,hidden : true}, 
						...
						{name : 'CONCLUSION',index : 'CONCLUSION',width : 200,align : "center",sorttype : "String",editable : true,hidden : true}, 
						{name : 'passInfo',index : 'passInfo',width : 100,align : "center",align : "center",sorttype : "String",editable : true} ],
			height : '100%',
			autowidth : true,
			...
			onSelectRow : function(id) {
				...
				if (passed == '错误') {
					...
					$("#logic_result_tb").setGridParam({url : '${ContextPath}/check/unitLogicCheck_getCheckResultList?discId='+ discId, datatype:'json'}).trigger("reloadGrid");
					...
					$("#result_warn_tb").setGridParam({url : '${ContextPath}/check/unitLogicCheck_getCheckWarnList?discId='+ discId+ "&onlyGetWarn=yes", datatype:'json'}).trigger("reloadGrid");
		...
		var inputDiscId = $('#discChange').children('option:selected').val();
		$("#logic_config_tb").setGridParam({url : '${ContextPath}/check/unitLogicCheck_getDiscsList?inputDiscId='+ inputDiscId, datatype:'json'}).trigger("reloadGrid");
		...
		$('#logic_result_tb').jqGrid(
		{
			...
			colModel : [ {name : 'id',index : 'id',width : 60,align : "center",hidden : true}, 
			             ...
			             {name : 'discId',index : 'discId',width : 30,align : "center"},
			             {name : 'entityId',index : 'entityId',width : 30,align : "center",hidden : true},
			             {name : 'entityChsName',index : 'entityChsName',width : 60,align : "center"}, 
			             {name : 'dataId',index : 'dataId',width : 30,align : "center",sorttype : "String",hidden : true}, 
			             {name : 'seqNo',index : 'seqNo',width : 30,align : "center",sorttype : "String"}, 
			...
			loadComplete : function() {
				...
				if (needLoadWarn) {
					...
					$("#result_warn_tb").setGridParam({
						url : '${ContextPath}/check/unitLogicCheck_getCheckWarnList?discId=' + needLoadWarnDiscId, datatype:'json'}).trigger("reloadGrid");
				}
			},
		...
		$('#result_warn_tb').jqGrid(
		{
			...
			colModel : [ {name : 'id',index : 'id',width : 60,align : "center",hidden : true}, 
			             ...
			             {name : 'entityChsName',index : 'entityChsName',width : 150,align : "center"}, 
			             {name : 'userId',index : 'userId',width : 100,align : "center",hidden : true}, 
			             {name : 'conclusion',index : 'conclusion',width : 200,align : "center",sorttype : "String",editable : true} ],
			height : '100%',
			autowidth : true,
		...
		$("#logic_config_tb").setGridParam({url : '${ContextPath}/check/unitLogicCheck_getDiscsList/', datatype:'json'}).trigger("reloadGrid");
		$.ajax({
			type : 'POST',
	...
	$("#logic_check_btn").click(function() {
		...
		currentSelectedDiscId = "";
		var inputDiscId = $('#discChange').children('option:selected').val();
		$.post('${ContextPath}/check/unitLogicCheck_startLogicCheck?inputDiscId='+ inputDiscId, function() {
		...
		$.ajax({
			...
			success : function(data) {
				...
				if (data == 'checked') {
					var inputDiscId = $('#discChange').children('option:selected').val();
					$("#logic_config_tb").setGridParam({
						url : '${ContextPath}/check/unitLogicCheck_getDiscsList?inputDiscId='
							+ inputDiscId, datatype:'json'}).trigger("reloadGrid");
						$("#showerror").show();
						$("#showerror").text("错误信息");
						$("#showwarn").show();
						$("#showwarn").text("警告信息");
					...
					alert_dialog("您还从没进行过逻辑检查，请点击开始检查按钮！");
				}
			},
			error : function() {
	...
	function passStyle() {
		...
		for (var i = 0; i <= ids.length; i++) {
			...
			case '通过':
				$('#logic_config_tb').setCell(ids[i], "passInfo", '通过', {
					color : 'green'
				});
				break;
			case '错误':
				$('#logic_config_tb').setCell(ids[i], "passInfo", '错误', {
					color : 'red'
				});
				break;
			case '错误、警告':
				$('#logic_config_tb').setCell(ids[i], "passInfo", '错误、警告', {
					color : 'red'
				});
				break;
			case '警告':
				$('#logic_config_tb').setCell(ids[i], "passInfo", '警告', {
					color : 'orange'
				});
				break;
			case '未检查':
				$('#logic_config_tb').setCell(ids[i], "passInfo", '未检查', {
					color : 'blue'
				});
				break;
			}
