...
var isEditable='1';
var tree_flag=true;
var collectWidth = $("#div_publicity_layout").width() - $("#hide_tree").width();
...
function OutTree()
{
	...
	$("#jq_collect_table").width(collectWidth);
	$("#jqGrid_collect_tb").setGridWidth(
			$("#jq_collect_table").width()-8);
}
/**
 * 移入tree
 */
function InTree()
{
    ...
    $("#jq_collect_table").width(collectWidth-$("#ztreeConllection").width());
    $("#jqGrid_collect_tb").setGridWidth(
			$("#jq_collect_table").width()-8);
}
/*tree需要的设置*/
...
function treeOnClick(event, treeId, treeNode)
{
	...
	{
		var commonUrl="${ContextPath}/publicity/viewPub/initPubJqgrid/"+treeNode.value;
		var config={url:commonUrl,dataType:'json',success:renderPubJqGrid};
...
//通过学科重新加载采集树
function loadTreeByDiscId(discId){
	discId = discId+"";
	if(discId!=""&&discId!="undefined"){
		$.commonRequest({
			url:'${ContextPath}/publicity/viewPub/initPubTreeByDiscId/'+discId,
			dataType:'json',
...
function loadTreeByCatId(catId){
	...
	if(catId!=""&&catId!="undefined"){
		$.commonRequest({
			url:'${ContextPath}/publicity/viewPub/initPubTreeByCatId/'+catId,
			dataType:'json',
			success: renderCollectTree
		});
	}else{
